...
function aniki_entry_footer() {
	...
	if ( 'post' === get_post_type() ) {
		...
		$categories_list = get_the_category_list( esc_html__( ', ', 'aniki' ) );
		if ( $categories_list && aniki_categorized_blog() ) {
			printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'aniki' ) . '</span>', $categories_list ); // WPCS: XSS OK.
		}

		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', esc_html__( ', ', 'aniki' ) );
		if ( $tags_list ) {
			printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'aniki' ) . '</span>', $tags_list ); // WPCS: XSS OK.
		}
	}
...
function aniki_categorized_blog() {
	if ( false === ( $all_the_cool_cats = get_transient( 'aniki_categories' ) ) ) {
		...
		set_transient( 'aniki_categories', $all_the_cool_cats );
	}
